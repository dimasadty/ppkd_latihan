...
if (isset($_POST['submit'])) {
    ...
    $result = $db_latihanujikom->prepare($query);
    ...
    if ($row = $result->fetch(PDO::FETCH_ASSOC)) {
        ...
        if (sha1($password) === $row['password']){
            ...
            if (isset($row["id_level"]) == 1) {
                ...
                header("Location: /latihanujikom/dashboard.php");
                exit;
            } else if (isset($row["id_level"]) == 2) {
                $_SESSION['name'] = $row['name']; // Example session variable
                $_SESSION['id_level'] = $row['id_level']; // Example session variable
    
                // Redirect to dashboard or another secure page
                header("Location: /latihanujikom/dashboardinstructor.php");
                exit;
            }
...
<head>
    ...
    <title>PPKD Jakarta Pusat - Login</title>
    <!-- plugins:css -->
                        ...
                        <div class="auth-form-light text-left py-5 px-4 px-sm-5">
                            ...
                            <form class="pt-3" method="post">
                                ...
                                <div class="form-group">
                                    <input name="email" type="email" class="form-control form-control-user" id="email" placeholder="Enter your email">
                                </div>
                                <div class="form-group">
                                    <input name="password" type="password" class="form-control form-control-user" id="password" placeholder="Enter your password">
                                </div>
